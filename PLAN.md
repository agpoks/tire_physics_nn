...
- **Documentation** (`docs/`, `.readthedocs.yaml`) — an eight-chapter theory tutorial,
  one per prior, with the derivations, the physical justification, generated figures
  and links to the guaranteeing tests; plus a quickstart, the experiment guide, the
  dataset reference and a `sphinx-autoapi` API reference. Figures are pre-generated by
  `docs/generate_figures.py` and committed, so the docs build needs neither torch nor a
  training run.
- **`papers/references.bib`** — the KIT dataset and the VeTyT publications were verified
  against primary sources; the TUM cargo-bike, RoboRacer and Q-Motion entries are
  marked `UNVERIFIED` rather than guessed, and §4.4 above must be updated when they are
  confirmed.

### Deviations from the plan as written, and why

1. **Combined slip uses the symmetric slip vector by default.** The textbook
   theoretical slip $(\kappa, \tan\alpha)/(1+\kappa)$ is not odd in $\kappa$, so it
   would break the P2 guarantee for the analytical and ParameterNet rungs and make the
   ablation comparison dishonest. The asymmetric form is available as
   `theoretical_slip=True` and the trade-off is documented in
   `physics/pacejka.pacejka_combined`.
2. **Bounded parameters guarantee a *closed* range, not an open one.** `softplus` and
   `sigmoid` saturate to their endpoints in float32, so the contract is
   `lo <= p <= hi` and every declared endpoint is itself a physically valid value
   (e.g. `sigma_min = 0.01 m > 0`).
3. **`ParameterTireNet` reports load-corrected friction.** Reporting the nominal $\mu$
   at $F_{z0}$ understated the ellipse at light load; `params["mu_x"]` is now
   $\mu(F_z)$ and `params["mu_x0"]` the nominal value.
4. **The graining state uses an exact zero-order-hold discrete update.** The $[0,1]$
   invariance of the continuous ODE only transfers to an explicit Euler step for small
   $\Delta t$; the exponential update makes the bound hold for any step size without
   clamping the gradient at the boundary.
5. **Thermal defaults retuned** to plausible racing-tire magnitudes and explicitly
   flagged as engineering estimates rather than identified values.
